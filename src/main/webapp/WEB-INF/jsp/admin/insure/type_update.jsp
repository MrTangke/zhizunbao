...
<div class=" clearfix">
 ...
 <div>
<form id="myForm" action="<%=basePath%>insure/updateType" method="post">
<input type="hidden" name="typeId" value="${type.typeId }"> 
   <ul class="add_conent">
	    <li class=" clearfix">
	    	<label class="label_name"><i><font color="red">*</font></i>类型名称：</label>
	    	<input name="typeName" value="${type.typeName }" type="text" class="add_text" datatype="*" nullmsg="不能为空" /></li>
	    <li class=" clearfix"><label class="label_name"><i><font color="red">*</font></i>所属分类：</label>
		    <select style="margin-left: 10px" name="pid" class="text_add">
			    ...
			    <c:forEach items="${topTypeList}" var="tt">
			   		<option ${tt.typeId==type.typeId?'selected':'' } value="${tt.typeId }">${tt.typeName }</option>
			    </c:forEach>
		    </select>
	    </li>
	    <li>
