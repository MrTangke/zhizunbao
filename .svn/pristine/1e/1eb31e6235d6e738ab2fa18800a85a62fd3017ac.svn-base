...
<body>
  <article class="page-container">
	<form style="width:650px" action="<%=basePath%>carouselimg/updateindeximg" class="form form-horizontal" enctype="multipart/form-data" id="Form" method="post">
		<div class="row cl">
			<label class="form-label col-xs-4 col-sm-3"><span class="c-red">*</span>图片位置：</label>
			<div class="formControls col-xs-8 col-sm-9">
			<input type="hidden" class="input-text"  name="imgId" value="${frontImg.imgId }"  size="50" datatype="*" />
				<select id="test" name="typeId">
							<c:forEach items="${list }" var="x">
								<option value="${x.typeId }"  ${frontImg.typeId ==x.typeId?'selected':''}>
										${x.typeName }
								</option>
							</c:forEach>
					</select>
		...
		<div class="row cl">
			...
			<div class="formControls col-xs-8 col-sm-9">
				<input type="text" class="input-text" name="imgOrder" value="${frontImg.imgOrder }"  size="50" placeholder="请输入图片顺序" datatype="aaa" />
			</div>
		</div>
			<div class="row cl">
				<label class="form-label col-xs-4 col-sm-3"><span class="c-red">*</span>图片跳转的路径：</label>
				<div class="formControls col-xs-8 col-sm-9">
					<input type="text" class="input-text" name="jumpUrl" value="${frontImg.jumpUrl }"  size="50" placeholder="请输入图片跳转的路径" />
				</div>
			</div>
		 <div class="row cl">
			<label class="form-label col-xs-4 col-sm-3"><span class="c-red">*</span>原始图片：</label>
			<div class="formControls col-xs-8 col-sm-9">
				<img src="<%=basePath %>${frontImg.imgUrl }" style="width: 150px;height: 150px"/>
			</div>
		</div>
		<div class="row cl">
