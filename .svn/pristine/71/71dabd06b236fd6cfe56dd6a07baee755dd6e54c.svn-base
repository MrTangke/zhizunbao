...
<body>
	<div class="margin clearfix">
		<div class="border clearfix">
			<span class="l_f"> <a
				href="<%=basePath%>navigation/toAddNav" id="Competence_add"
				class="btn btn-warning" title="添加导航条目"><i class="fa fa-plus"></i>
					添加导航条目</a>
			</span> <span class="r_f">
			</span>
		</div>
				...
				<c:forEach items="${nav}" var="nav">
					<tbody>
						<tr>
							<td>${nav.navName}</td>
							<td>${nav.navAdress}</td>
							<td><a title="编辑" onclick="Competence_modify(${nav.navId})"
								href="<%=basePath %>navigation/toUpdateNavigationById?nav_id=${nav.navId}"
								class="btn btn-xs btn-info"><i class="icon-edit bigger-120"></i></a>
								<a title="删除"
								href="<%=basePath %>navigation/deleteNavigationById?nav_id=${nav.navId}"
								onclick="Competence_del(this,'1')"
