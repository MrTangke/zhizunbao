...
(function($, win, undef) {
	...
	var Validform = function(forms, settings, inited) {
		...
		forms.each(function() {
			...
			Validform.util.enhance.call($this, curform.settings.tiptype,
					curform.settings.usePlugin, curform.settings.tipSweep);
			curform.settings.btnSubmit
					&& $this.find(curform.settings.btnSubmit).bind("click",
							function() {
								$this.trigger("submit");
								return false
							});
			$this.submit(function() {
				var subflag = Validform.util.submitForm.call($this,
						curform.settings);
				subflag === undef && (subflag = true);
				return subflag
			});
			$this.find("[type='reset']").add(
					$this.find(curform.settings.btnReset)).bind("click",
					function() {
	...
	Validform.util = {
		...
		enhance : function(tiptype, usePlugin, tipSweep, addRule) {
			...
			curform
					...
					.each(
							function() {
								if (tiptype == 2) {
									if ($(this).parents(".formControls").next()
											.find(".Validform_checktip").length == 0) {
										$(this)
												.parents(".formControls")
												.next()
												.append(
														"<span class='Validform_checktip' />");
										$(this).siblings(".Validform_checktip")
												.remove()
									}
								} else {
		...
		getNullmsg : function(curform) {
			...
			label = obj.siblings(label).eq(0).text()
					|| obj.siblings().find(label).eq(0).text()
					|| obj.parent().siblings(label).eq(0).text()
					|| obj.parent().siblings().find(label).eq(0).text();
		...
		_regcheck : function(datatype, gets, obj, curform) {
			...
			if (reg.test(datatype)) {
				...
				if (Validform.util.toString
					...
					passed = Validform.util.dataType[datatype](gets, obj,
							curform, Validform.util.dataType);
		...
		showmsg : function(msg, type, o, triggered) {
			...
			if (type == 2 && o.obj) {
				o.obj.parents(".formControls").next().find(
						".Validform_checktip").html(msg);
				Validform.util.cssctl(o.obj.parents(".formControls").next()
						.find(".Validform_checktip"), o.type)
			}
		...
		check : function(curform, subpost, bool) {
			...
			if (ajaxurl && !Validform.util.isEmpty.call($(this), inputval)
				...
				Validform.util.showmsg.call(curform, curform.data("tipmsg").c
					...
					success : function(data) {
						if ($.trim(data.status) === "y") {
							...
							Validform.util.showmsg.call(curform, inputobj
									.attr("sucmsg")
									|| curform.data("tipmsg").r || tipmsg.r,
									settings.tiptype, {
										obj : inputobj,
										type : 2,
										sweep : settings.tipSweep
		...
		submitForm : function(settings, flg, url, ajaxPost, sync) {
			...
			if (flag) {
				...
				if (settings.ajaxPost || ajaxPost === "ajaxPost") {
					...
					ajaxsetup.url = url || ajaxsetup.url || settings.url
							|| curform.attr("action");
					Validform.util.showmsg.call(curform,
	...
	Validform.prototype = {
			...
			$(obj.forms).each(
					function() {
						...
						Validform.util.enhance.call($this,
								this.settings.tiptype, this.settings.usePlugin,
		...
		ajaxPost : function(flag, sync, url) {
			...
			$(obj.forms).each(
					function() {
						...
						Validform.util.submitForm.call($(obj.forms[0]),
								this.settings, flag, url, "ajaxPost", sync)
					});
			return this
		},
		...
		check : function(bool, selector) {
			...
			curform.find(selector).each(
					function() {
						Validform.util.check.call(this, curform, "", bool)
								|| (flag = false)
					});
			return flag
		},
